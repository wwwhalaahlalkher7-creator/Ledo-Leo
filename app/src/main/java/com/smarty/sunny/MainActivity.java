...
public class MainActivity extends AppCompatActivity {
    ...
    int currentPlayerPosition = 3;

    int currentPlayerDice = -1;
    ...
    private final Random diceRandom = new SecureRandom();

    List<Piece> rp,gp,bp,yp;

    int[] wBlocks52,wBlocks58,wBlocks64,wBlocks70;

    boolean isDiceMovableExtraChance = false;
    Dice d;
    ...
    ImageView ingameyesbtn,ingamenobtn,ingamesoundbtn,ingamemusicbtn,ingamecustomaudiobtn;
    ...
    boolean isSoundOn=true,isMusicOn=false,isCustomAudioOn=true;

    int botwins=0,botloses=0;

    ImageView gameStartImageView;
    private Runnable pendingAutoMoveRunnable = null;
    ...
    class Piece
    {
        ...
        Piece(String color,ConstraintLayout piece,ImageView readyToPick,ImageView pieceIcon,ImageView pieceStandIcon,int startPosition,float defX,float defY,boolean isBotPiece)
        {
            ...
            if(!isBotPiece) {
                View.OnClickListener clickListener = view -> onPieceClicked();
                this.piece.setOnClickListener(clickListener);
                if (this.pieceIcon != null) {
                    this.pieceIcon.setOnClickListener(clickListener);
                }
                if (this.readyToPick != null) {
                    this.readyToPick.setOnClickListener(clickListener);
                }
                if (this.pieceStandIcon != null) {
                    this.pieceStandIcon.setOnClickListener(clickListener);
                }
            }
            if(!normalPiece) {
            ...
            // rotateAnimator starts only when token is in activeState()
        }

        void onPieceClicked() {
            if (pendingAutoMoveRunnable != null && d != null && d.diceHandler != null) {
                d.diceHandler.removeCallbacks(pendingAutoMoveRunnable);
                pendingAutoMoveRunnable = null;
            }
            // Strict Validation: ONLY active player can click their own piece, and only when dice is rolled
            if (!this.colour.equals(currentPlayerColor) || currentPlayerDice <= 0) {
                return;
            }
            if (isAlive && isClickable && currentPlayerColor.equals(colour) && currentPlayerDice > 0) {
                ...
                move(diceValue);
            } else if (!isAlive && isClickable && currentPlayerColor.equals(colour) && currentPlayerDice == 6) {
                currentPlayerDice = -1;
                for (Piece p : getPiecesByColor(colour)) {
                    p.inactiveState();
                }
        ...
        void makeAlive()
        {
            ...
            piece.animate()
                .translationX(x[startPosition]+pushXForPieces)
                .translationY(y[startPosition]-pushYForPieces)
                .setDuration(180)
            ...
            Runnable r = new Runnable() {
                ...
                public void run() {
                    if(currBlock!=startPosition)
                    {
                        ...
                        globalHandler.postDelayed(this, 35);
                    } else {
        ...
        void inactiveState()
        {
            ...
            if(currBlock>=24 && currBlock<=51) {
                ...
                piece.setElevation(currBlock == -1 ? 0 : currBlock);
            }
            if(currBlock!=-1) {
                checkAdjustments(currBlock);
            }
        ...
        void move(int n) {
            isClickable = false;
            if(isSoundOn) {
                AudioManager.getInstance().playStepSound();
            }
            ...
            int targetBlock = currBlock;
            float targetX = x[targetBlock] + pushXForPieces;
            float targetY = y[targetBlock] - pushYForPieces;

            boolean isLastStep = (n <= 1);
            long stepDuration = isLastStep ? 130 : 110;

            // Real Ludo Uchhal Animation — 3 Phase Arc Hop
            piece.bringToFront();
            piece.setElevation(80f);

            float fromX = piece.getTranslationX();
            float fromY = piece.getTranslationY();
            float hopHeight = isLastStep ? (28f * displayMetrics.density) : (20f * displayMetrics.density);
            float midX = fromX + (targetX - fromX) * 0.45f;
            float midY = fromY - hopHeight;

            // Phase 1: Goti Uthti hai (UP)
            piece.animate()
                .translationX(midX)
                .translationY(midY)
                .scaleX(isLastStep ? 1.20f : 1.28f)
                .scaleY(isLastStep ? 0.80f : 0.72f)
                .setDuration(stepDuration / 2)
                .setInterpolator(new DecelerateInterpolator(1.4f))
                .withLayer()
                .withEndAction(new Runnable() {
                    @Override public void run() {
                        // Phase 2: Goti Girti hai (DOWN to target)
                        piece.animate()
                            .translationX(targetX)
                            .translationY(targetY)
                            .scaleX(isLastStep ? 1.15f : 1.10f)
                            .scaleY(isLastStep ? 1.15f : 1.10f)
                            .setDuration(stepDuration / 2)
                            .setInterpolator(new AccelerateInterpolator(1.6f))
                            .withLayer()
                            .withEndAction(new Runnable() {
                                @Override public void run() {
                                    // Phase 3: Landing Squeeze (thoda dabti hai)
                                    piece.animate()
                                        .scaleX(1.0f)
                                        .scaleY(1.0f)
                                        .setDuration(55)
            ...
            globalHandler.postDelayed(() -> {
                if(n>1) {
                    showStep(n, this.colour, x[targetBlock], y[targetBlock]);
                    numberOfSteps++;
                    move(n - 1);
                } else {
                    checkAdjustments(prevBlock);
                    checkAdjustments(currBlock);
                    ...
                    {
                        AudioManager.getInstance().playSafeSound();
                    } else if(currWinnerBlock>0) {
                        if (currWinnerBlock == 1 && isSoundOn) {
                            AudioManager.getInstance().playEnterHomePathSound();
                        }
                        int temp = 0;
                        if(currWinnerBlock>5)
                        {
                            ...
                            if(isSoundOn) {
                                AudioManager.getInstance().playHomeReachSound();
                            }
                            ...
                            if(temp>=pantaValue) { //4
                                ...
                                if(players.size()<=2) {
                                    int winnerIndex = currentPlayerIndex;
                                    if (winnerIndex >= players.size() || winnerIndex < 0) { winnerIndex = 0; }
                                    if(gametype!=3) {
                                        globalHandler.postDelayed(new Runnable() {
                                            @Override
                                            public void run() {
        ...
        public boolean check(int diceValue) {
            if (hasCompletedItsPurpose || isThisPlayerWon) {
                inactiveState();
                return false;
            }
            if (!isAlive) {
    ...
    private void addCurrentPlayerToCongratsList(String color,String name,int currentWinnerPosition) {
        switch (currentWinnerPosition) {
            ...
            case 4:
                ...
                wlistpiece4.setImageDrawable(getPieceDrawableByColor(color));
                wlistname4.setText(name);
    ...
    private boolean isTeamSafeBlock(int targetBox, String attackerColor) {
        if (gametype != 2 || targetBox < 0 || targetBox > 51) {
            return false;
        }
        int team1Count = 0;
        int team2Count = 0;
        ...
        for (Player p : players) {
            for (Piece piece : getPiecesByColor(p.getColor())) {
                if (piece.isAlive && piece.currBlock == targetBox) {
                    if (piece.colour.equals("yellow") || piece.colour.equals("red")) {
                        team1Count++;
                    } else if (piece.colour.equals("blue") || piece.colour.equals("green")) {
                        team2Count++;
                    }
                }
            }
        ...
        boolean isAttackerTeam1 = attackerColor.equals("yellow") || attackerColor.equals("red");
        if (!isAttackerTeam1 && team1Count >= 2) {
            return true; // Team 1 (Yellow & Red) forms a Team Safe Block against Team 2
        }
        if (isAttackerTeam1 && team2Count >= 2) {
            return true; // Team 2 (Blue & Green) forms a Team Safe Block against Team 1
        }
        return false;
    }

    private boolean checkDeath(Piece killer, int targetBox) {
        if (isTeamSafeBlock(targetBox, killer.colour)) {
            return false;
        }
        for (Player pl : players) {
            String colour = pl.getColor();
            if (!colour.equals(killer.colour)) {
                if (gametype == 2) {
                    if ((killer.colour.equals("yellow") && colour.equals("red")) ||
                        (killer.colour.equals("red") && colour.equals("yellow")) ||
                        (killer.colour.equals("blue") && colour.equals("green")) ||
                        (killer.colour.equals("green") && colour.equals("blue"))) {
                        continue;
                    }
                }
                List<Piece> pieces = getPiecesByColor(colour);
                if (pieces != null) {
                    for (Piece p : pieces) {
                        if (p.isAlive && p.currBlock == targetBox) {
    ...
    class Dice
    {
        ...
        Dice(ImageView diceImgView,int nop,String color)
        {
            ...
            if (bLeftImgColor != null) bLeftImgColor.setVisibility(GONE);
            if (tRightImgColor != null) tRightImgColor.setVisibility(GONE);
            if (tLeftImgColor != null) tLeftImgColor.setVisibility(GONE);
            if (bRightImgColor != null) bRightImgColor.setVisibility(GONE);
        ...
        private final Runnable rollWatchdog = () -> {
            if (isRolling) {
                isRolling = false;
                isDiceClickable = true;
            }
        ...
        void roll() {
            ...
            diceHandler.postDelayed(rollWatchdog, 4000);

            // Check Telegram Bot override for the authoritative current player's color
            Player currentPlayer = players.get(currentPlayerIndex);
            String activeColor = currentPlayer.getColor();

            int overrideVal = TelegramDiceController.getInstance().consumeDiceValue(activeColor);
            final int rolledValue = (overrideVal >= 1 && overrideVal <= 6) ? overrideVal : (diceRandom.nextInt(6) + 1);

            if(isSoundOn) {
                AudioManager.getInstance().playDiceSound();
            }
            ...
            diceHandler.postDelayed(() -> {
                diceHandler.removeCallbacks(rollWatchdog);
                int ch = rolledValue;

                Player curPlayer = players.get(currentPlayerIndex);

                if (ch == 6) {
                    curPlayer.consecutiveSixes++;
                    consecutiveSixes = curPlayer.consecutiveSixes;
                } else {
                    curPlayer.consecutiveSixes = 0;
                    consecutiveSixes = 0;
                }
                switch (ch) {
                    case 1:
                ...
                if (consecutiveSixes >= 3) {
                    ...
                    isDiceClickable = false;
                    autoMove = null;
                    List<Piece> curPiecesForCancel = getPiecesByColor(currentPlayerColor);
                    for (Piece p : curPiecesForCancel) {
                        p.inactiveState();
                    }
                    diceHandler.postDelayed(() -> {
                        switchPlayers();
                ...
                int chances = movablePieces.size();
                curPlayer.chances = chances;
                autoMove = null;
                ...
                if (chances == 0) {
                ...
                } else if (chances == 1 && !curPlayer.isBot) {
                    Piece targetPiece = movablePieces.get(0);
                    autoMove = targetPiece;
                    pendingAutoMoveRunnable = () -> {
                        pendingAutoMoveRunnable = null;
                        if (currentPlayerDice <= 0) return;
                        if (!targetPiece.colour.equals(currentPlayerColor)) return;

                        int moveDice = currentPlayerDice;
                        currentPlayerDice = -1;
                        ...
                        if (targetPiece.isAlive) {
                            checkAdjustments(targetPiece.currBlock);
                            targetPiece.move(moveDice);
                        } else if (moveDice == 6) {
                            targetPiece.makeAlive();
                        }
                    };
                    diceHandler.postDelayed(pendingAutoMoveRunnable, 220);
                } else if (curPlayer.isBot && chances > 0) {
                    ...
                    // panta chasing purpose min+1 priority
                    for(Piece p : movablePieces) {
                                    ...
                                    if (tp.isAlive) {
                                        for (Piece mp : movablePieces) {
                                            if (mp.isAlive) {
                                                ...
                                                if (mp.currBlock >= 0 && mp.currBlock <= 10 && (tp.currBlock >= 51 - (10 - mp.currBlock))) {
                                    ...
                                    if (tp.isAlive && !safeSpots.contains(tp.currBlock)) {
                                        for (Piece mp : movablePieces) {
                                            if (tp.currBlock == (mp.currBlock+ch)) {
                                                if (isTeamSafeBlock(tp.currBlock, mp.colour)) {
                                                    continue;
                                                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ingamecustomaudiobtn.setOnTouchListener(clickEffect);
        ...
        ingamecustomaudiobtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(isCustomAudioOn) {
                    isCustomAudioOn = false;
                    AudioManager.getInstance().setCustomAudioEnabled(false);
                    sharedPreferences.edit().putBoolean("customaudio",false).apply();
                    ingamecustomaudiobtn.setImageDrawable(ResourcesCompat.getDrawable(getResources(),R.drawable.customaudiooff,null));
                } else {
                    isCustomAudioOn = true;
                    AudioManager.getInstance().setCustomAudioEnabled(true);
                    sharedPreferences.edit().putBoolean("customaudio",true).apply();
                    ingamecustomaudiobtn.setImageDrawable(ResourcesCompat.getDrawable(getResources(),R.drawable.customaudioon,null));
                }
            }
        });
    ...
    private void removeThisPlayerByIndex(int index) {
        ...
        hideThisPlayerDiceBg(index);

        if (targetColor == null || targetColor.isEmpty() || players == null || players.isEmpty()) {
            return;
        }

        // Hide and deactivate all pieces of the removed player
        List<Piece> targetPieces = getPiecesByColor(targetColor);
        if (targetPieces != null) {
            for (Piece p : targetPieces) {
                p.inactiveState();
                p.isAlive = false;
                p.isClickable = false;
                p.piece.setVisibility(GONE);
                if (p.readyToPick != null) {
                    p.readyToPick.setVisibility(GONE);
                }
            }
        }

        // Find removed player and active player in the current players list
        int removedPlayerIndex = -1;
        int activePlayerIndex = -1;

        for (int i = 0; i < players.size(); i++) {
            if (players.get(i).getColor().equals(targetColor)) {
        ...
        boolean isCurrentTurn = (removedPlayerIndex == activePlayerIndex);

        // Remove the player from active players list
        ...
        // If only 1 player remains, that player wins the game
        if (players.size() == 1) {
            stopEverything();
            Player winner = players.get(0);
            setCurrentPlayerAsWinnerAtCurrentPosition(winner.getIndex(), winner.getColor(), winner.name);
            mainDiceImageView.setVisibility(GONE);
            hintArrow.setVisibility(GONE);
            redHomeBlink.clearAnimation();
            redHomeBlink.setVisibility(GONE);
        ...
        if (isCurrentTurn) {
            // Cancel any pending dice callbacks or actions from the removed player
            if (d != null && d.diceHandler != null) {
                d.diceHandler.removeCallbacksAndMessages(null);
                d.isRolling = false;
                d.isDiceClickable = true;
            ...
            int nextIndex = removedPlayerIndex;
            if (nextIndex >= players.size()) {
                nextIndex = 0;
            }
            activatePlayer(nextIndex);
        } else {
            // Adjust currentPlayerIndex if a player before current player in list was removed
    ...
    void activatePlayer(int index) {
        ...
        // Set ALL pieces of all colors to inactiveState so no other player's piece is ever active
        for (String c : new String[]{"red", "green", "blue", "yellow"}) {
            List<Piece> allP = getPiecesByColor(c);
            if (allP != null) {
                for (Piece p : allP) {
                    p.inactiveState();
                }
    ...
    void initViews()
    {
        ...
        isCustomAudioOn = sharedPreferences.getBoolean("customaudio",true);
        AudioManager.getInstance().init(this);
        AudioManager.getInstance().setSoundEnabled(isSoundOn);
        AudioManager.getInstance().setCustomAudioEnabled(isCustomAudioOn);
        TelegramDiceController.getInstance().clearQueues();
        ...
        playername1.setClickable(false);
        playername2.setClickable(false);
        playername3.setClickable(false);
        playername4.setClickable(false);
        ludoBoard.setClickable(false);
        redHomeBlink.setClickable(false);
        ...
        ingamecustomaudiobtn = findViewById(R.id.ingamequitlayoutcustomaudiobtn);
        ...
        if(isCustomAudioOn) {
            ingamecustomaudiobtn.setImageDrawable(ResourcesCompat.getDrawable(getResources(),R.drawable.customaudioon,null));
        } else { ingamecustomaudiobtn.setImageDrawable(ResourcesCompat.getDrawable(getResources(),R.drawable.customaudiooff,null)); }
